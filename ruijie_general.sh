...
first_encode=$(echo -n "$SERVICE" | curl -Gso /dev/null -w %{url_effective} --data-urlencode @- "" | cut -c 3-)
ENCODED_SERVICE=$(echo -n "$first_encode" | curl -Gso /dev/null -w %{url_effective} --data-urlencode @- "" | cut -c 3-)

first_encode=$(echo -n "$Password" | curl -Gso /dev/null -w %{url_effective} --data-urlencode @- "" | cut -c 3-)
ENCODED_Password=$(echo -n "$first_encode" | curl -Gso /dev/null -w %{url_effective} --data-urlencode @- "" | cut -c 3-)
